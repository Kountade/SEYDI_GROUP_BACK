...
class HasAgenceAccess(BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if isinstance(obj, Transfer):
            has_from_access = request.user.peut_acceder_agence(
                obj.from_agence_id)
...
# ✅ MIS À JOUR : Inclut maintenant le rôle comptable
...
# ✅ NOUVEAU : Permission pour les comptables
class IsComptable(BasePermission):
    """Permission pour les comptables (ou supérieur)"""
    ...
    def has_permission(self, request, view):
        ...
        if hasattr(request.user, 'a_role_dans_agence'):
            agences = request.user.get_agences()
            for agence in agences:
                if request.user.a_role_dans_agence(agence.id, 'comptable'):
                    return True
...
# ✅ NOUVEAU : Permission combinée pour les rôles de gestion
class IsGestionnaireOuComptable(BasePermission):
    """
    Permission pour les gestionnaires de stock ou comptables (ou supérieur)
    """
        ...
        if hasattr(request.user, 'a_role_dans_agence'):
            ...
            for agence in agences:
                if request.user.a_role_dans_agence(agence.id, 'gestionnaire_stock') or \
                   request.user.a_role_dans_agence(agence.id, 'comptable'):
                    return True

        return False


# ✅ NOUVEAU : Permission pour tous les rôles opérationnels
class IsPersonnelAgence(BasePermission):
    """
    Permission pour tout le personnel d'agence (chef, commercial, gestionnaire, comptable)
    """
    ...
    def has_permission(self, request, view):
        ...
        if hasattr(request.user, 'a_role_dans_agence'):
            agences = request.user.get_agences()
            for agence in agences:
                if request.user.a_role_dans_agence(agence.id, 'chef_agence') or \
                   request.user.a_role_dans_agence(agence.id, 'commercial') or \
                   request.user.a_role_dans_agence(agence.id, 'gestionnaire_stock') or \
                   request.user.a_role_dans_agence(agence.id, 'comptable'):
                    return True
...
# ✅ MIS À JOUR : Inclut maintenant les rôles comptables
...
# ✅ NOUVEAU : Permission pour la gestion comptable
class CanManageAccounting(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if hasattr(request.user, 'a_role_dans_agence'):
            agences = request.user.get_agences()
            for agence in agences:
                if request.user.a_role_dans_agence(agence.id, 'comptable') or \
                   request.user.a_role_dans_agence(agence.id, 'chef_agence'):
                    return True
...
# ✅ NOUVEAU : Permission pour les rapports et analyses
...
# ✅ NOUVEAU : Permission pour la gestion des utilisateurs (RH)
class CanManageUsers(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if hasattr(request.user, 'a_role_dans_agence'):
            ...
            for agence in agences:
                if request.user.a_role_dans_agence(agence.id, 'chef_agence'):
                    # Chef d'agence peut gérer les utilisateurs de son agence
                    return True

        return False

    def has_object_permission(self, request, view, obj):
        """Vérification au niveau de l'objet pour la gestion des utilisateurs"""
        if not request.user or not request.user.is_authenticated:
            return False
        ...
        # Si l'objet est un utilisateur, vérifier qu'il est dans la même agence
...
# ✅ NOUVEAU : Permission pour les opérations financières
class CanManageFinances(BasePermission):
    """Permission pour gérer les opérations financières"""
    ...
    def has_permission(self, request, view):
        ...
        if request.user.has_perm('users.can_manage_finances'):
            return True
