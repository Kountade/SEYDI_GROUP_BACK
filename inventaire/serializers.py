    ...
    def validate(self, data):
        if data.get('movement_type') in ['out', 'transfer']:
            ...
            if product and product.stock_quantity < quantity:
                raise serializers.ValidationError(f"Stock insuffisant. Disponible: {product.stock_quantity}")
        ...
        if data.get('movement_type') == 'transfer':
            ...
            if from_wh and to_wh and from_wh == to_wh:
                raise serializers.ValidationError("Les entrepôts source et destination doivent être différents")
        return data
...
class TransferCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('reference', 'created_at', 'updated_at', 'created_by', 'validated_by', 
                           'completed_date', 'approved_by', 'approved_at', 'rejected_reason')
    ...
    def validate(self, data):
        ...
        if not from_agence or not to_agence:
            raise serializers.ValidationError("Les deux agences sont requises")
        if from_agence == to_agence:
            raise serializers.ValidationError("Les agences source et destination doivent être différentes")
        if from_agence.type_agence != 'principale':
            raise serializers.ValidationError({"from_agence": "L'agence source doit être une agence principale"})
        if to_agence.type_agence != 'secondaire':
            raise serializers.ValidationError({"to_agence": "L'agence destination doit être une agence secondaire"})

        user = self.context['request'].user
        if not user.peut_acceder_agence(to_agence.id):
            raise serializers.ValidationError("Vous devez être rattaché à l'agence secondaire destinataire")
        if not items_data:
            raise serializers.ValidationError({"items": "Au moins un article est requis"})

        for i, item in enumerate(items_data):
            if not item.get('product'):
                raise serializers.ValidationError({"items": f"Article {i+1}: le produit est requis"})
            quantity = item.get('quantity', 0)
            if quantity <= 0:
                raise serializers.ValidationError({"items": f"Article {i+1}: la quantité doit être positive"})
            unit_price = item.get('unit_price', 0)
            if unit_price <= 0:
                raise serializers.ValidationError({"items": f"Article {i+1}: le prix unitaire doit être positif"})
    ...
    def create(self, validated_data):
        ...
        transfer = Transfer.objects.create(**validated_data, created_by=user)
