# products/views.py
# Version complète et corrigée avec gestion des images
...
class ProductViewset(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        """
        Met à jour un produit avec gestion de la suppression d'image
        """
        partial = kwargs.pop('partial', False)
        instance = self.get_object()

        # Vérifier si l'image principale doit être supprimée
        remove_main_image = request.data.get('remove_main_image', 'false')
        if remove_main_image.lower() == 'true' and instance.main_image:
        ...
        if 'main_image' in request.FILES and instance.main_image:
            if os.path.isfile(instance.main_image.path):
                try:
                    os.remove(instance.main_image.path)
                except Exception as e:
    ...
    def prices(self, request, pk=None):
        ...
        if user.est_pdg() or user.est_drh():
            ...
            warehouses = Warehouse.objects.filter(
                agence_id__in=agences_ids, is_active=True)
        ...
        serializer = ProductPricingSerializer(
            prices, many=True, context={'request': request})
        return Response(serializer.data)
    ...
    def remove_image(self, request, pk=None):
        ...
        try:
            ...
            # Vérifier si c'était l'image principale
            was_main = image.is_main

            # Supprimer l'image (la méthode delete() de ProductImage supprime le fichier)
            image.delete()

            # Si c'était l'image principale, mettre à jour le produit
            if was_main:
                # Chercher une autre image pour la définir comme principale
                next_image = product.images.filter(is_active=True).first()
                if next_image:
    ...
    def remove_main_image(self, request, pk=None):
        """
        Supprime l'image principale d'un produit
        URL: /products/{id}/remove_main_image/
        """
        product = self.get_object()

        if not product.main_image:
            return Response(
                {'error': 'Aucune image principale à supprimer'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            # Si une image de la galerie était marquée comme principale, la désactiver
            product.images.filter(is_main=True).update(is_main=False)
    ...
    def set_main_image(self, request, pk=None):
        ...
        URL: /products/{id}/set_main_image/
        """
        product = self.get_object()
        image_id = request.data.get(
            'image_id') or request.query_params.get('image_id')
    ...
    @action(detail=True, methods=['post'])
    def replace_main_image(self, request, pk=None):
        """
        Remplace l'image principale d'un produit par une nouvelle
        URL: /products/{id}/replace_main_image/
        """
        product = self.get_object()
        image_file = request.FILES.get('image')
        ...
        try:
            ...
            return Response({
                'message': 'Image principale remplacée avec succès',
                'main_image': request.build_absolute_uri(product.main_image.url)
            }, status=status.HTTP_200_OK)

        except Exception as e:
            return Response(
                {'error': f'Erreur lors du remplacement: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def duplicate(self, request, pk=None):
        ...
        serializer = ProductDetailSerializer(
            duplicate_product, context={'request': request})
        return Response(serializer.data, status=status.HTTP_201_CREATED)
